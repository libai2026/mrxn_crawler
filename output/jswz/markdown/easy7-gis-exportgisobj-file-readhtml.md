...
该系统的/Easy7/rest/gis/exportGisObj 和 /Easy7/rest/gisCore/exportGisObj接口存在前台任意
[文件读取](https://mrxn.net/tag/%E6%96%87%E4%BB%B6%E8%AF%BB%E5%8F%96)
漏洞，攻击者通过构造恶意路径参数（如WEB-INF/web.xml）可读取服务器上的任意文件，可能导致敏感信息泄露（如系统配置文件、用户凭证等）。由于天地伟业产品多用于关键基础设施领域，若存在公网暴露实例，可能带来严重的安全风险。
...
再来看本次的漏洞接口 /Easy7/rest/gis/exportGisObj 和 /Easy7/rest/gisCore/exportGisObj （这是审计时额外发现的，漏洞通告只有前者，可能是不同版本的区别）的对应方法
`exportGisObj()`
的实现逻辑

其中一个路径来自
`com.tiandy.easy7.core.rest.CLS_REST_Gis#exportGisObj`
...
另一个路径来自
`com.tiandy.easy7.core.rest.CLS_REST_GisCore#exportGisObj`
二者实现是一样的，只是来自不同的接口而已。

其中
`request.getRealPath("/")`
获取的结果是当前应用的根目录，
`voObjGisObj.getFileName()`
返回的是用户传递的
`fileName`
参数；
...
到这里，这个文件读取漏洞的成因就非常清楚了：用户请求传递
`fileName`
参数，被直接拼接到
`new FileInputStream(fileUrl)`
fileUrl 部分进行文件操作，整个过程无任何校验或过滤，因此造成任意
[文件读取](https://mrxn.net/tag/%E6%96%87%E4%BB%B6%E8%AF%BB%E5%8F%96)
漏洞。
...
![天地伟业Easy7 exportGisObj 文件读取漏洞](https://image.mrxn.net/eb519b8b10cb43558215a7bb5bd9946b.webp)
...
文章链接：
<https://mrxn.net/jswz/easy7-gis-exportGisObj-file-read.html>
