...
福建科立讯通信指挥调度管理平台是一个专门针对通信行业的管理平台。福建科立讯通信指挥调度管理平台 api/get\_sos/items.php 接口处存在
...
虽然最终执行的时候使用了 pdo.prepare 方法预处理执行SQL，但是没有使用参数绑定传参，而是直接将
`$_REQUEST['usernumber']`
直接被嵌入到 SQL 查询中，而没有经过任何过滤或转义，然后执行拼接后的SQL语句，等于卵用，最终造成SQL注入
...
![福建科立讯通信指挥调度管理平台 api/get_sos/items.php SQL注入漏洞](https://image.mrxn.net/414bc50049d740828e9ebdfa26d49464.webp)
...
文章标题：
[福建科立讯通信指挥调度管理平台 api/get\_sos/items.php SQL注入漏洞](https://mrxn.net/jswz/api-get_sos-items-usernumber-sqli.html)
  
文章链接：
<https://mrxn.net/jswz/api-get_sos-items-usernumber-sqli.html>
