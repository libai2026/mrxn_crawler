...
U8 Cloud MARosterPhotoServlet 接口处存在SQL注入漏洞，未经身份验证的远程攻击者除了可以利用 SQL 注入漏洞获取数据库中的信息（例如，管理员后台密码、站点的用户个人信息）之外，甚至在高权限的情况可向服务器中写入木马，进一步获取服务器系统权限。
...
通告可知SQL注入点在 MARosterPhotoServlet 接口

![用友U8 Cloud MARosterPhotoServlet SQL注入漏洞](https://image.mrxn.net/d5c8d7f33f014d05980bec12194da4a4.webp)
...
调试也可以看到SQL语句传入直接拼接到SQL语句中，最终调用 (RosterPhoto)dao.executeQuery 执行拼接后的SQL语句，造成
[SQL注入](https://mrxn.net/tag/sql%E6%B3%A8%E5%85%A5 "SQL注入")
漏洞。

![用友U8 Cloud MARosterPhotoServlet SQL注入漏洞](https://image.mrxn.net/93faafeee7fd4f6988d1b95e760a821f.webp)

根据补丁对比查看可知，修复方式是参数化+预编译处理来预防SQL注入漏洞。

![用友U8 Cloud MARosterPhotoServlet SQL注入漏洞](https://image.mrxn.net/afb2de0cde0b4a1392ebbab6f75ba7ea.webp)
...
![用友U8 Cloud MARosterPhotoServlet SQL注入漏洞](https://image.mrxn.net/ca221ba2b1854d0e854651292cd20c8e.webp)
...
文章链接：
<https://mrxn.net/jswz/yonyou-u8cloud-MARosterPhotoServlet-sqli.html>
