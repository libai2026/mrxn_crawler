...
博斯外贸管理软件是杭州博斯有限公司推出的一款针对外贸业务的管理软件。博斯外贸管理软件V6.0
`DCreceiveBox.jsp`
接口多个参数均存在
...
如果SAVE、SAVE不等于空时，SAVE、NOWKEY 拼接进SQL语句，造成
...
如果 USERKF 等于空，则进入else 执行SQL语句，SuserId ==> nowUserId 拼接进SQL语句，造成
[SQL注入](https://mrxn.net/tag/SQL注入)
。
...
当 setCustom=YJZY 是，authes不包含 $ ,authes、fxlx 拼接进SQL语句造成
[SQL注入](https://mrxn.net/tag/SQL注入)
。
...
当 setAUTOGJ=Y时，NOWGJ 直接拼接进SQL语句造成
[SQL注入](https://mrxn.net/tag/SQL注入)
。
...
不管 USERKF 是否为空，sortkey 均直接拼接进order by 语句SQL语句造成SQL注入
[漏洞](https://mrxn.net/tag/%E6%BC%8F%E6%B4%9E)
（需要注意 order by 语句后的列名必须是存在的才可注入）。
...
![博斯外贸管理软件 DCreceiveBox.jsp 多处SQL注入漏洞](https://image.mrxn.net/b1555eefe27647849fdb7d45fcd5bec9.webp)
...
文章链接：
<https://mrxn.net/jswz/51boss-crm-module-DCreceiveBox-sqli.html>
...
text: encodeURI("https://mrxn.net/jswz/51boss-crm-module-DCreceiveBox-sqli.html"),
width: 100,
height: 100,
colorDark: "#000000",
colorLight: "#ffffff",
