...
![用友U8Cloud /u8cloud/openapi/ce.paper.query SQL 注入漏洞](https://image.mrxn.net/f960771a7fa8434d95a4165e827e2eb1.webp)

我搭建好环境以及mcp后，提出需求，其余均为agent自动完成。

![用友U8Cloud /u8cloud/openapi/ce.paper.query SQL 注入漏洞](https://image.mrxn.net/d6d492e25d8348e5b07460184370ec2e.webp)
...
U8 Cloud 系统的
`/u8cloud/openapi/ce.paper.query`
接口中发现一处严重的高危
[SQL注入](https://mrxn.net/tag/SQL%E6%B3%A8%E5%85%A5 "SQL注入")
漏洞。该漏洞允许未经身份验证的远程攻击者通过构造恶意的 JSON 请求，在数据库中执行任意 SQL 命令。漏洞的根源在于服务端未对用户输入的
`pk_group`
和
`pk_paper`
参数进行任何过滤或预编译处理，直接将其拼接至 SQL 查询语句中。

* **漏洞类型**
  : SQL 注入 (Blind/Stacked Queries)
* **影响组件**
  : 用友 U8 Cloud (ce.paper.query 接口)
* **危险等级**
  : 高危 (Critical)
* **利用条件**
  :
...
1. **Servlet**
   **映射 (
   ...
   1. 在
      ...
      `/u8cloud/openapi/*`
      被映射到
      `nc.bs.framework.server.extsys.ExtSystemInvokerServlet`
      。
   2. 该 Servlet 的
      `doAction`
      方法会拦截请求。它通过遍历
...
2. **服务分发 (
   ...
   1. `ExtSystemInvokerServlet`
      ...
      的服务组件。
   2. 该服务映射到
      `u8c.server.APIOpenServletForJSON`
      类。
   3. `APIOpenServletForJSON.doAction`
      将请求转发给
      `u8c.server.APIOpenController`
      处理。
3. **路径解析与 Action 映射 (
   `APIOpenController`
   )**
   ...
   5. 框架根据
      ...
      `billMark`
      或配置）将请求路由到
      `u8c.bs.ce.action.PaperQueryAction`
      。
...
1. **Action 处理 (
   ...
   1. `PaperQueryAction`
      继承自
      `AbstractBatchSaveAPIPubVOAction<PaperExecuteVO>`
      。
   ...
   3. **关键点**
      ...
      `{"paperexecute": [{"pk_group": "...", "pk_paper": "..."}]}`
      。
   4. 框架使用反射将 JSON 数组中的字段（如
      `pk_group`
      ,
      `pk_paper`
      ）映射到
      `PaperExecuteVO`
      对象的对应属性。此时，恶意的 SQL 注入 payload (如
...
2. **漏洞触发点 (
   ...
   1. 在
      `u8c.bs.ce.dmo.PaperResultDMO.java`
      的
      `query`
      方法中，存在直接的字符串拼接：
...
3. 数据库（如 SQL Server）支持堆叠查询（Stacked Queries），因此会先执行前面的 SELECT 语句，紧接着执行
   `WAITFOR DELAY`
   命令，导致数据库暂停响应 5 秒。
...
![用友U8Cloud /u8cloud/openapi/ce.paper.query SQL 注入漏洞](https://image.mrxn.net/b5eb5ac37cbe4f638f0a8fe680140d8b.webp)
...
文章链接：
<https://mrxn.net/jswz/u8cloud-openapi-ce-paper-query-sqli.html>
