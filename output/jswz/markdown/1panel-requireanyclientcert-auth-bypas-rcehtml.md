---
title: "CVE-2025-54424：1Panel 客户端证书绕过RCE漏洞 一体化工具 (扫描+利用)"
source: https://mrxn.net/jswz/1panel-RequireAnyClientCert-auth-bypas-rce.html
---

# CVE-2025-54424：1Panel 客户端证书绕过RCE漏洞 一体化工具 (扫描+利用)

[Mrxn](https://mrxn.net/author/1)* 发表于2025/8/4 20:34
* 1795浏览
* [2评论](#comment)
* 2小时
  阅读
...
受影响版本中，Agent 端 TLS 认证策略为 tls.RequireAnyClientCert，仅要求提供证书但不验证其可信性。攻击者可通过自签名证书绕过 TLS 校验，并伪造CN字段为panel\_client，绕过应用层校验。最终攻击者可伪造证书进行未授权
...
> cert.subject\_org=="FIT2CLOUD"&&ip.port="9999” || cert.subject.suffix=="panel\_server"
>
> cert.subject.org="FIT2CLOUD" && port="9999" && protocol="tls" || cert.subject.cn="panel\_server"
...
* 而Core端与Agent端通讯所使用的https协议，在证书校验中未完全校验证书的真实性导致接口未授权。1panel中由于存在大量命令执行或高权限的接口，导致
  [RCE](https://mrxn.net/tag/rce)
  。

![CVE-2025-54424：1Panel 客户端证书绕过RCE漏洞 一体化工具 (扫描+利用)](https://image.mrxn.net/82cf38e6ff67452cbf0ad0367170b807.webp)

### 代码审计过程

1. 首先我们进入到Agent HTTP路由文件
   `agent/init/router/router.go`

![CVE-2025-54424：1Panel 客户端证书绕过RCE漏洞 一体化工具 (扫描+利用)](https://image.mrxn.net/c4b7ffb226334c0480b6708553e045e0.webp)
...
2. 发现
   ...
   ![CVE-2025-54424：1Panel 客户端证书绕过RCE漏洞 一体化工具 (扫描+利用)](https://image.mrxn.net/ac994a0d3fcc462daaee756c72ed1831.webp)
3. 发现
   `Certificate`
   函数判断了
   ...
   ![CVE-2025-54424：1Panel 客户端证书绕过RCE漏洞 一体化工具 (扫描+利用)](https://image.mrxn.net/bbefc70884ca4ecc84530fe5a1ea9cf1.webp)
4. 由于
   `c.Request.TLS.HandshakeComplete`
   的真假判断是通过
   ...
   ![CVE-2025-54424：1Panel 客户端证书绕过RCE漏洞 一体化工具 (扫描+利用)](https://image.mrxn.net/8297f74142cd41dc902e77f81b8bdcdd.webp)
...
5. 后续进入
   `Certificate`
   函数中的其他判断，只验证了证书CN字段为panel\_client，未验证证书签发者。最后发现WebSocket连接可以绕过Proxy-ID验证。
     
   ![CVE-2025-54424：1Panel 客户端证书绕过RCE漏洞 一体化工具 (扫描+利用)](https://image.mrxn.net/fcefb2d277b2409cb761c6a399db454d.webp)
...
![CVE-2025-54424：1Panel 客户端证书绕过RCE漏洞 一体化工具 (扫描+利用)](https://image.mrxn.net/35be1ba15fc34a1ba8614fa4d271ca4a.webp)
...
* Terminal SSH WebSocket 接口（根据上述问题可执行任意命令）
  ...
  ![CVE-2025-54424：1Panel 客户端证书绕过RCE漏洞 一体化工具 (扫描+利用)](https://image.mrxn.net/dd78a5ca2ea64b31bdaaa56c637c339b.webp)
...
2、burp 加载生成的panel\_client.crt和panel\_client.key后，打开ws请求，设置目标开始请求
...
![CVE-2025-54424：1Panel 客户端证书绕过RCE漏洞 一体化工具 (扫描+利用)](https://image.mrxn.net/78fdcbfa324d4dbca49c53cf56c9701b.webp)
...
文章标题：
[CVE-2025-54424：1Panel 客户端证书绕过RCE漏洞 一体化工具 (扫描+利用)](https://mrxn.net/jswz/1panel-RequireAnyClientCert-auth-bypas-rce.html)
  
文章链接：
<https://mrxn.net/jswz/1panel-RequireAnyClientCert-auth-bypas-rce.html>
...
text: encodeURI("https://mrxn.net/jswz/1panel-RequireAnyClientCert-auth-bypas-rce.html"),
width: 100,
height: 100,
colorDark: "#000000",
colorLight: "#ffffff",
