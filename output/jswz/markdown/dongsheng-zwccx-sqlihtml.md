...
东胜物流软件是青岛东胜伟业软件有限公司一款集订单管理、仓库管理、运输管理等多种功能于一体的物流管理软件。东胜物流信息管理系统 Areas/Mobile/Views/WMS/ZWCCX.aspx 接口存在
...
1. **过滤失效**
   ...
   过滤危险字符，但
   `String.Replace()`
   方法返回新字符串，必须将返回值赋值才有效。当前代码未赋值，导致过滤
   **完全无效**
   。
...
1. **字符串拼接**
   **SQL**
   ：使用字符串插值
   `$"{str1}"`
   直接将用户输入拼接到 SQL 语句中，未使用参数化查询。
2. **直接执行文本**
   **SQL**
   ：通过
   `ExecuteReader((CommandType) 1, commandText)`
   执行，
   `CommandType`
   为 1 即
   `CommandType.Text`
...
![东胜物流软件 ZWCCX.aspx SQL注入漏洞](https://image.mrxn.net/81a9af97c27a481daf0a9633451b8728.webp)
...
文章标题：
[东胜物流软件 ZWCCX.aspx SQL注入漏洞](https://mrxn.net/jswz/dongsheng-ZWCCX-sqli.html)
  
文章链接：
<https://mrxn.net/jswz/dongsheng-ZWCCX-sqli.html>
...
text: encodeURI("https://mrxn.net/jswz/dongsheng-ZWCCX-sqli.html"),
width: 100,
height: 100,
colorDark: "#000000",
colorLight: "#ffffff",
