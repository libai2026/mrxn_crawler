...
是万户网络协同办公产品多年来一直将主要精力致力于中高端市场的一款OA协同办公软件产品，统一的基础管理平台，实现用户数据统一管理、权限统一分配、身份统一认证。统一规划门户网站群和协同办公平台，将外网信息维护、客户服务、互动交流和日常工作紧密结合起来，有效提高工作效率。万户 ezOFFICE
`govdocumentmanager_judge_receivenum.jsp`
接口存在
...
直接看
`jboss/jboss-as/server/oa/deploy/defaultroot.war/modules/govoffice/gov_documentmanager/govdocumentmanager_judge_receivenum.jsp`
...
* `numId`
  ...
  以及后续追加的
  `recordId`
  参数均未经过任何过滤或转义，直接拼接至SQL语句，存在明显的SQL注入风险。
* 攻击者可通过构造恶意参数（如
  `numId=1 OR 1=1 --`
  ），实现SQL注入攻击，导致数据泄露、篡改或破坏。
...
* `recordId`
  参数在未做任何合法性校验、类型检查或转义的情况下，直接拼接至SQL语句中。

整体执行流程如下

![万户ezOFFICE govdocumentmanager_judge_receivenum.jsp SQL注入漏洞](https://image.mrxn.net/91fd07900b6d4137b4c2b3f0431c8be3.webp)
...
![万户ezOFFICE govdocumentmanager_judge_receivenum.jsp SQL注入漏洞](https://image.mrxn.net/8542ec24fbbd4e9f8f8819c3013c8654.webp)
...
文章标题：
[万户ezOFFICE govdocumentmanager\_judge\_receivenum.jsp SQL注入漏洞](https://mrxn.net/jswz/defaultroot-ezOFFICE-govdocumentmanager_judge_receivenum-numId-sqli.html)
  
文章链接：
<https://mrxn.net/jswz/defaultroot-ezOFFICE-govdocumentmanager_judge_receivenum-numId-sqli.html>
...
text: encodeURI("https://mrxn.net/jswz/defaultroot-ezOFFICE-govdocumentmanager\_judge\_receivenum-numId-sqli.html"),
width: 100,
height: 100,
colorDark: "#000000",
colorLight: "#ffffff",
