...
直接看
`VisitorMapConfigController`
里关于
`updateVisitorMapConfig`
的实现
...
用户输入
`visitorPoliceTpm.cardId`
和
`visitorPoliceTpm.img`
→ 经
`trim()`
处理后，
`cardId`
...
`imgData`
→ 传递到
`GetPhoto.generateImageByBase64`
方法 →
`imgData`
解码后写入文件，
`imgFilePath`
...
![汉王e脸通综合管理平台 updateVisitorMapConfig.do 任意文件上传漏洞](https://image.mrxn.net/3521c08d9e56489eb2360abdffadc4aa.webp)
...
文章链接：
<https://mrxn.net/jswz/hanvon-efacego-updateVisitorMapConfig-upload-rce.html>
