...
[![利用file_put_contents()函数写shell的小trick](../content/uploadfile/201709/1c6e1505914096.png "点击查看原图")](../content/uploadfile/201709/1c6e1505914096.png)

思路大致如下：

file\_put\_contents()在写入文件时的第二个参数可以传入数组，如果是数组的话，将被连接成字符串再进行写入。在正则匹配前，传入的是一个数组。得益于PHP的弱类型特性，数组会被强制转换成字符串，也就是
**Array**
，
**Array**
肯定是满足正则
**\A[ \_a-zA-Z0-9]+\z**
的，所以不会被拦截。这样就可以绕过类似检测
*“<?”*
之类的waf。
...
[![利用file_put_contents()函数写shell的小trick](../content/uploadfile/201709/bbb71505914096.png "点击查看原图")](../content/uploadfile/201709/bbb71505914096.png)

代码检测了写入的内容是否存在“<”“>”“?”等字符。根据上面的trick，我们可以通过传入一个数组来达到写入shell的目的。可以看到虽然有个警告。但config.php确实被写入了。<? php phpinfo(); 如下所示:
[![利用file_put_contents()函数写shell的小trick](../content/uploadfile/201709/12031505914096.png "点击查看原图")](../content/uploadfile/201709/12031505914096.png)
...
文章标题：
[利用file\_put\_contents()函数写shell的小trick](https://mrxn.net/jswz/php-file_put_contents_w-shell.html)
  
文章链接：
<https://mrxn.net/jswz/php-file_put_contents_w-shell.html>
