...
Western Digital MyCloud NAS是一款网络附加存储设备，旨在提供集中存储和共享解决方案。它允许用户在家中或办公室通过网络访问文件，支持多种设备的备份和共享。Western Digital MyCloud NAS snmp\_mgr.cgi中存在多处命令执行漏洞，攻击者可通过该漏洞在服务器端任意
[执行命令](https://mrxn.net/tag/rce)
，获取服务器权限，进而控制整个web服务器。
...
![西部数据 NAS  snmp_mgr.cgi 多处命令执行漏洞](https://image.mrxn.net/80c04e784a934a5da5c89522e4234767.webp)

程序首先调用
`cgiFormString`
函数，从HTTP请求中获取一个名为
`cmd`
的参数值。这个值决定了接下来要执行什么操作。

根据漏洞通告，使用IDA打开 snmp\_mgr.cgi 搜索
**cgi\_SNMPv3\_delete\_one\_record**
定位到它的处理逻辑处

![西部数据 NAS  snmp_mgr.cgi 多处命令执行漏洞](https://image.mrxn.net/f86c9910081c47d198418113390fc71d.webp)
...
当
**cmd=cgi\_SNMPv3\_delete\_one\_record**
时，跳转进入
**sub\_117E4**
，汇编处理逻辑如下
...
到这就很清楚了，漏洞形成原因是直接将 uid 拼接进 snmp\_tool 命令工具的参数里面，从而导致的
[命令注入漏洞](https://mrxn.net/tag/rce)
。
...
也是将 uid 直接拼接进 snmp\_tool 命令工具的参数里面，从而导致的
...
程序依次调用
`cgiFormString`
函数来获取HTTP请求中的多个参数值，并将它们存储到栈上的不同缓冲区中。
...
![西部数据 NAS  snmp_mgr.cgi 多处命令执行漏洞](https://image.mrxn.net/323faa194b9d4fd4b9d2223e96be2451.webp)
...
此函数的功能是接收一个包含多项配置的XML-like数据块，解析它，然后用解析出的值构造一个复杂的命令行来配置SNMPv3用户。与之前分析的函数类似，这个函数也存在
**极其严重的命令注入漏洞**
，因为解析出的用户数据在未经充分过滤的情况下被直接拼接到
`system()`
调用中。
...
2. **数据解析 (Data Parsing)**
   : 这是该函数的核心部分。程序假设
   `data`
   参数是一个包含多个字段的结构化字符串（可能是简化的XML或类似格式）。它通过
   **多次调用**
   `sub_10F34`
   **函数**
   来解析出各个字段的值。
   1. `sub_10F34`
      **的作用**
      : 这个子函数
      `sub_10F34(char *data_blob, int field_index, char *output_buffer)`
      的功能是从输入的
      `data_blob`
      中，根据
      `field_index`
      （字段索引，如1, 2, 3...）提取出第 N 个
      `<cell>`
      标签内的内容，并将其复制到
      `output_buffer`
      中。
...
3. **不充分的过滤 (Insufficient Filtering)**
   ...
   3. **关键缺陷**
      : 正如之前分析，
      `sub_11000`
      函数的过滤机制是无效的，它无法阻止命令注入。
      **更重要的是，其他5个解析出的字段完全没有经过任何过滤！**
4. **构造并执行命令 (Command Construction and Execution)**
   :
   1. `sprintf`
      : 程序使用
      `sprintf`
      将所有7个解析出的字段值拼接到一个非常长的命令行格式字符串
      `aSnmpToolV3USLS`
      中。
...
![西部数据 NAS  snmp_mgr.cgi 多处命令执行漏洞](https://image.mrxn.net/0cf79e89877c456fbc3209ae2a3e0d58.webp)
...
![西部数据 NAS  snmp_mgr.cgi 多处命令执行漏洞](https://image.mrxn.net/fbc4e23d14354d32ad2b6e7963ebca3f.webp)

访问生成的文件，成功创建

## cgi\_set\_SNMP\_v2

![西部数据 NAS  snmp_mgr.cgi 多处命令执行漏洞](https://image.mrxn.net/b9df59916e58408bb4120fd30dea88f1.webp)

![西部数据 NAS  snmp_mgr.cgi 多处命令执行漏洞](https://image.mrxn.net/9718b80c03ae49f8b6d6eb49235bd99f.webp)

成功执行命令

## cgi\_SNMPv3\_modify\_one\_record

![西部数据 NAS  snmp_mgr.cgi 多处命令执行漏洞](https://image.mrxn.net/0c3d5ef7edc543269eae1ceb0ea4404f.webp)

![西部数据 NAS  snmp_mgr.cgi 多处命令执行漏洞](https://image.mrxn.net/502dfb92c88d48a0916a58e0b2cf4a31.webp)

## cgi\_set\_SNMPv3\_one\_record

![西部数据 NAS  snmp_mgr.cgi 多处命令执行漏洞](https://image.mrxn.net/ba9f173a99c34ee0949f3864bbe993e7.webp)

![西部数据 NAS  snmp_mgr.cgi 多处命令执行漏洞](https://image.mrxn.net/73e44dcc2d93483dbcbadfc739f62ad4.webp)
...
文章标题：
[西部数据 NAS snmp\_mgr.cgi 多处命令执行漏洞](https://mrxn.net/jswz/west-cgi-bin-snmp_mgr-rce.html)
  
文章链接：
<https://mrxn.net/jswz/west-cgi-bin-snmp_mgr-rce.html>
...
text: encodeURI("https://mrxn.net/jswz/west-cgi-bin-snmp\_mgr-rce.html"),
width: 100,
height: 100,
colorDark: "#000000",
colorLight: "#ffffff",
